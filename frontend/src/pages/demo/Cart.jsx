...
export const Cart = () => {
  ...
  //   cartData.map((elm) => console.log(elm, 'checking data'))
  const user = JSON.parse(localStorage.getItem('User'))

  useEffect(() => {
    axios
      .get(`http://localhost:8080/cart/62ac04ec9815f5858901538d`)
      .then((data) => {
        console.log(data.data[0].cart, 'latest added data')
        setCart(data[0].cart)
      })
      .catch((e) => console.log('somthing went wrong in fetching data'))
